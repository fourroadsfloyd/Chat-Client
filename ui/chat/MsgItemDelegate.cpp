      ...
      m_senderFontMetrics(m_senderFont)
{
    ...
    // 更新 FontMetrics
    m_contentFontMetrics = QFontMetrics(m_contentFont);
    m_timeFontMetrics = QFontMetrics(m_timeFont);
    m_senderFontMetrics = QFontMetrics(m_senderFont);
}
                                 ...
                                 const QModelIndex &index) const
{
    ...
    // 使用 QTextDocument 计算文本实际宽度（动态）
    QTextDocument doc;
    doc.setDefaultFont(m_contentFont);
    doc.setPlainText(content);
    doc.setTextWidth(MAX_WIDTH); // 先设置最大宽度来计算实际需要的宽度

    // 获取文本实际需要的宽度（不超过 MAX_WIDTH）
    qreal textWidth = doc.documentLayout()->documentSize().width();
    int actualBubbleWidth = qMin(MAX_WIDTH, (int)qCeil(textWidth)) + PADDING * 2;

    // 重新设置文本宽度为实际宽度，用于计算高度
                                       ...
                                       const QString &content, const QString &time) const
{
    ...
    QFontMetrics fm(m_senderFont);
    int senderWidth = fm.horizontalAdvance(sender);
