...
MsgInput::MsgInput(QWidget *parent)
    : QTextEdit(parent)
{
...
QVector<MsgInfo> MsgInput::getMsgList()
{
    mGetMsgList.clear();
...
void MsgInput::dragEnterEvent(QDragEnterEvent *event)
{
...
void MsgInput::dropEvent(QDropEvent *event)
{
    insertFromMimeData(event->mimeData());
    event->accept();
}

void MsgInput::keyPressEvent(QKeyEvent *e)
{
    if((e->key()==Qt::Key_Enter || e->key()==Qt::Key_Return) && !(e->modifiers() & Qt::ShiftModifier))
    {
        emit send();    //发给谁了？尚未使用该功能
        return;
    }
    QTextEdit::keyPressEvent(e);
}

void MsgInput::insertFileFromUrl(const QStringList &urls)
{
    if(urls.isEmpty())
        return;
...
void MsgInput::insertImages(const QString &url)
{
    QImage image(url);
    //按比例缩放图片
...
void MsgInput::insertTextFile(const QString &url)
{
...
bool MsgInput::canInsertFromMimeData(const QMimeData *source) const
{
    return QTextEdit::canInsertFromMimeData(source);
}

void MsgInput::insertFromMimeData(const QMimeData *source)
{
...
bool MsgInput::isImage(QString url)
{
...
void MsgInput::insertMsgList(QVector<MsgInfo> &list, QString flag, QString text, QPixmap pix)
{
    MsgInfo msg;
    msg.msgFlag=flag;
    msg.content = text;
...
QStringList MsgInput::getUrl(QString text)
{
    QStringList urls;
...
QPixmap MsgInput::getFileIconPixmap(const QString &url)
{
...
QString MsgInput::getFileSize(qint64 size)
{
    QString Unit;
    double num;
    if(size < 1024){
